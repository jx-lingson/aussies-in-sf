import {resolveArea} from '../neighbourhoods';
...
export async function POST(r:Request){
 ...
 try{
  ...
  let resolved;try{resolved=await resolveArea(Object.fromEntries(form))}catch(e){return fail(e instanceof Error?e.message:'Choose a valid neighbourhood centre.')}const p={...parsed.data,area:resolved.name},db=database(),bucket=photos(),id=owner?.profile.id||crypto.randomUUID(),token=owner?.token||newToken();
  ...
  const write=db.prepare('INSERT INTO profiles (id,name,title,email,linkedin,area,status,arrival,departure,created_at,photo_key,owner_key) VALUES (?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(id) DO UPDATE SET name=excluded.name,title=excluded.title,email=excluded.email,linkedin=excluded.linkedin,area=excluded.area,status=excluded.status,arrival=excluded.arrival,departure=excluded.departure,photo_key=excluded.photo_key,owner_key=excluded.owner_key').bind(id,p.name,p.title,p.email,p.linkedin,p.area,p.status,p.status==='visitor'?p.arrival:null,p.status==='visitor'?p.departure:null,Date.now(),photoKey,await digest(token));await db.batch(resolved.statement?[resolved.statement,write]:[write]);
  key=null;if(bytes&&owner?.profile.photo_key)await bucket.delete(owner.profile.photo_key).catch(console.error);
  return Response.json({ok:true,area:p.area},{headers:{'Set-Cookie':ownerCookie(r,token),'Cache-Control':'no-store'}});
