...
export async function POST(r:Request){
 ...
 try{
  ...
  const duplicate=await db.prepare("SELECT id FROM profiles WHERE id != ? AND (lower(email)=lower(?) OR lower(rtrim(linkedin,'/'))=lower(rtrim(?,'/')))").bind(id,p.email,p.linkedin).first();
  if(duplicate)return fail('A profile with this email or LinkedIn already exists. Open its private edit link or use the browser where you created it.',409);
